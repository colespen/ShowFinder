...
import type { Performer, Show } from "../types/show.ts";
 ...
 */
export function venueKey(show: Show): string {
...
export function artistKey(show: Show): string {
 ...
 */
function isSameShow(a: Show, b: Show): boolean {
...
function mergePerformers(
  preferred: Performer[] | undefined,
  other: Performer[] | undefined,
): Performer[] {
 ...
 */
function mergeShow(preferred: Show, other: Show): Show {
...
export function mergeShows(
  ...sources: (Show[] | undefined)[]
): { data: Show[]; duplicates: number } {
  const records: Show[] = [];
  const byVenue = new Map<string, number>();
  const byArtist = new Map<string, number>();
  let duplicates = 0;

  const claim = (show: Show, venue: string, artist: string): void => {
