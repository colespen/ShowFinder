...
import {
  ...
  TicketmasterEventAttraction,
  TicketmasterEvent,
} from "../models/ticketmaster.type.js";

const debug = Debug("sf-api:service:ticketmaster");

class TicketMasterService {
  static async eventSearch(
    geoPoint: string,
    dateStart: Moment,
    dateEnd: Moment
  ): Promise<[Moment, Moment, TicketmasterEventData]> {
  ...
  static parseEventData(rawData: any): TicketmasterEventData {
    ...
    for (let event of rawData?._embedded.events) {
      const parsedVenues: Array<TicketmasterEventVenue> =
        event._embedded.venues?.map((vnu: any) => {
      ...
      const parsedAttractions: Array<TicketmasterEventAttraction> =
        event._embedded.attractions?.map((att: any) => {
          ...
          if (
            att.externalLinks?.spotify &&
            Array.isArray(att.externalLinks.spotify) &&
            att.externalLinks.spotify.length > 0
          ) {
          ...
          if (
            att.externalLinks?.homepage &&
            Array.isArray(att.externalLinks.homepage) &&
            att.externalLinks.homepage.length > 0
          ) {
